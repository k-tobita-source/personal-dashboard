...
- DB ファイルは **`~/.my-kanban/kanban.db`** などユーザー領域に配置（リポジトリ外）。
...
| 種別                             | 保管先                                             | Git 除外               |
| -------------------------------- | -------------------------------------------------- | ---------------------- |
| 外部APIトークン（Google/Slack）  | `~/.my-kanban/credentials.json` 等（ユーザー領域） | 対象外（リポジトリ外） |
| アプリ設定（OAuth client id 等） | `.env`（リポジトリ内・gitignore済）                | ✅ `.gitignore`        |
| ボード状態・取得データ           | `~/.my-kanban/kanban.db`（SQLite）                 | リポジトリ外           |
