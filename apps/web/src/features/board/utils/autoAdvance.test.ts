...
describe("selectAutoMoves", () => {
  it("schedule で開始到達したタスクは in_progress へ（source 問わず）", () => {
    const todo = task({
      id: "a",
      source: "todo",
      startAt: new Date(2026, 5, 16, 11, 0),
    });
    const cal = task({
      id: "b",
      source: "calendar",
      startAt: new Date(2026, 5, 16, 11, 0),
      endAt: new Date(2026, 5, 16, 13, 0),
    });
  ...
  it("終了超過の calendar は done へ（schedule からも in_progress からも）", () => {
    const inSchedule = task({
      id: "a",
      source: "calendar",
      lane: "schedule",
      startAt: new Date(2026, 5, 16, 9, 0),
      endAt: new Date(2026, 5, 16, 10, 0),
    });
    const inProgress = task({
      id: "b",
      source: "calendar",
      lane: "in_progress",
      startAt: new Date(2026, 5, 15, 9, 0),
      endAt: new Date(2026, 5, 15, 10, 0),
    });
  ...
  it("終了超過でも todo は done にしない（schedule なら開始到達で in_progress）", () => {
    ...
    expect(selectAutoMoves([todo], now)).toEqual([
      { id: "a", lane: "in_progress" },
    ]);
  });
