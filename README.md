...
personal-dashboard/
├─ apps/
│  └─ web/                # @acme/web   Next.js（カンバン UI + tRPC ルート + Google OAuth コールバック）
└─ packages/
   ├─ api/                # @acme/api          tRPC router + service 層（UI / MCP 共有）
   ├─ db/                 # @acme/db           Drizzle ORM + SQLite スキーマ / マイグレーション
   ├─ integrations/       # @acme/integrations Google / Slack / Gmail コネクタ（読み取り専用）
   ├─ mcp/                # @acme/mcp          MCP サーバー（stdio）
   └─ ui/                 # @acme/ui           shadcn/ui + Tailwind コンポーネント
```
